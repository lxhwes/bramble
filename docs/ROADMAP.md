...
- Dockerfile (multi-stage `node:22-bookworm-slim` for the `better-sqlite3` native build) plus `docker-compose.yml` with a single service and a SQLite volume. Migrations run at container start. Documented env vars including required `ORIGIN` (adapter-node CSRF on form POSTs).
