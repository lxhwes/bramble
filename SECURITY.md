...
- **Self-host requires `ORIGIN`.** adapter-node rejects cross-origin form POSTs unless `ORIGIN` matches the public URL. This affects the SvelteKit form actions — **session create and shortlist add/remove**. Voting is a JSON `fetch`, which the origin check does not cover, so a wrong `ORIGIN` presents as "swiping works but I can't start a session". Misconfiguring it breaks writes rather than exposing data, but leaving it at the localhost default in production is an operator error worth calling out.
