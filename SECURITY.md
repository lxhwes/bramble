...
- **Write surface is rate-limited.** Vote append and session create are limited per IP (Cloudflare WAF on the hosted instance; an in-process limiter on the self-host Node target). The Node limiter is per-process — a multi-replica self-host deployment needs a reverse-proxy rate limit (e.g. nginx `limit_req`) in front, and must set `ADDRESS_HEADER`/`XFF_DEPTH` so the limiter keys on the real client IP rather than the proxy's.
